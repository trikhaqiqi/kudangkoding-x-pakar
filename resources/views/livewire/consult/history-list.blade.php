<section class="space-y-6">
                            ...
                            @forelse ($history as $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right font-medium space-x-2">
                                        <a href="{{ route('consult_summary', ['id' => $item->id]) }}"
                                            class="btn-blue py-1 px-3 text-sm inline-block">Detail</a>
                                        @role('user')
                                            @if ($show)
                                                <button
                                                    wire:click="showDeleteForm({{ $item->id }}, '{{ $loop->iteration }}')"
                                                    class="btn-danger py-1 px-3 text-sm inline-block">
                                                    {{ __('Delete') }}
                                                </button>
                                            @endif
                                        @endrole
    ...
    <x-jet-confirmation-modal wire:model="modalDelete">
        ...
        <x-slot name="content">
            ...
            <b>{{ __('consultation number -') }}{{ $item_position }}</b> ?
            {{ __('After an consultation history is deleted, all its resources and data will be permanently deleted.') }}
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-2" wire:click="deleteHistory({{ $history_id }})"
